...
def login:
    ...
    faces = face_cascade.detectMultiScale(gray_image,scaleFactor=l.l,minNeighbors=5, minSize=(30,30))
...
for name,filename in registered_data.items():
    ...
    if len(registered_face_encodings) >0 and len(login_face_encodings)>0 :
        matches = face_recognition.compare_faces(registered_face_encodings,login_face_encodins[0])
